...
SurfaceRemesher6<ScalarType>::SurfaceRemesher6()
{
	this->Clear();
}
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::Clear()
{
	m_InputMesh.Clear();
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::ClearInternalData()
{
...
template<typename ScalarType>
bool SurfaceRemesher6<ScalarType>::CheckInput()
{
	if (m_InputMesh.CheckIfTriangleMesh() == false)
	{
		MDK_Error("m_InputMesh is NOT TriangleMesh @ SurfaceRemesher6::CheckInput()")
		return false;
	}

	if (m_InputMesh.Check_If_DataStructure_is_Clean() == false)
	{
		MDK_Error("m_InputMesh DataStructure is NOT Clean @ SurfaceRemesher6::CheckInput()")
		return false;
	}
	...
	for (auto it = m_InputMesh.GetIteratorOfPoint(); it.IsNotEnd(); ++it)
	{
		...
		{
			MDK_Error("AdjacentPointCount >= 8 @ SurfaceRemesher6::CheckInput()")
			return false;
		}
	}
	return true;
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::Update()
{
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::InitilizeCandidateMesh()
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::InitilizeSmallTriangleMesh()
{// add middle point of each edge
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::GenerateCandidate()
{
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::GenerateTriangleCandidate_Type1()
{
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::GenerateQuadCandidate_Type2()
{
	for (auto it = m_InputMesh.GetIteratorOfEdge(); it.IsNotEnd(); ++it)
	{
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::GenerateQuadCandidate_Type2(EdgeHandleType EdgeHandle_input)
{
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::GenerateQuadCandidate_Type3_Type4()
{
	for (auto it = m_InputMesh.GetIteratorOfEdge(); it.IsNotEnd(); ++it)
	{
		auto tempIndex_preseve_edge = m_FeatureEdgeOfInputMesh.ExactMatch("first", it.GetEdgeHandle());
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::GenerateQuadCandidate_Type3_Type4(EdgeHandleType EdgeHandle_input)
{
	auto AdjacentFaceHandleList = m_InputMesh.Edge(EdgeHandle_input).GetAdjacentFaceHandleList();
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::Build_CandidateConflictTable_Overlap()
{
	...
	for (int_max k = 0; k < CandidateCount; ++k)
	{
		...
		{
			MDK_Error("somthing is wrong @ SurfaceRemesher6::Build_CandidateConflictTable_Overlap() ")
		}
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::Build_CandidateConflictTable_TJunction()
{
	auto PointCount_input = m_InputMesh.GetPointCount();
	auto EdgeCount_input = m_InputMesh.GetEdgeCount();
	auto CandidateCount = m_CandidateMesh.GetFaceCount();
	...
	{	
		...
		{
			MDK_Error("somthing is wrong @ SurfaceRemesher6::EvaluateCandidate() ")
		}
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::EvaluateCandidate()
{
	m_CandidateScoreList.Clear();
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::SelectCandidate()
{ 
	...
	//------------------ the seed -----------------------------------------------------------//
	FaceHandleType Candidate_seed;
	Candidate_seed.SetIndex(CandidateIndexList_sort[0]);
	m_CandidateIndicatorList[CandidateIndexList_sort[0]] = 1;

	this->BuildMixedTriQuadMesh();
	String TestDataPath = "C:/Research/Mesh/Remeshing/TestData/";
	SavePolygonMeshAsVTKFile(m_OutputMesh_Mixed, TestDataPath + "Region_Growing" + std::to_string(0) + ".vtk");
    //--------------------------------------------------------------------------------------------//
	DenseVector<FaceHandleType> SelectedCandidateAtRegionBoundary;//Candidate Handle on m_CandidateMesh
	SelectedCandidateAtRegionBoundary.Append(Candidate_seed);

	int_max Counter = 0;

	bool Flag_OutputMesh_Completed = false;
	while (Flag_OutputMesh_Completed == false)
	{
		DenseVector<FaceHandleType> AvailableCandidateList;
		AvailableCandidateList.SetCapacity(1000);
		for (int_max k = 0; k < SelectedCandidateAtRegionBoundary.GetLength(); ++k)
		{
			auto AdjacentCandidateHandleList = m_CandidateMesh.Face(SelectedCandidateAtRegionBoundary[k]).GetNeighbourFaceHandleList();
			auto AdjacentCandidateIndexList = this->ConvertHandleToIndex(AdjacentCandidateHandleList);
			auto ScoreList = m_CandidateScoreList.GetSubSet(AdjacentCandidateIndexList);
			auto IndexList_sort = ScoreList.Sort("descend");
			for (int_max n = 0; n < AdjacentCandidateHandleList.GetLength(); ++n)
			{
				if (this->Check_If_Candidate_is_Available(m_CandidateIndicatorList, AdjacentCandidateIndexList[n]) == true)
				{
					AvailableCandidateList.Append(AdjacentCandidateHandleList(IndexList_sort[n]));
				}
				else
				{
					m_CandidateIndicatorList[AdjacentCandidateIndexList[n]] = 0;
				}
			}			
		}

		if (AvailableCandidateList.IsEmpty() == true)
		{
			Flag_OutputMesh_Completed = true;
			break;
		}

		SelectedCandidateAtRegionBoundary.FastResize(0);

		// add candidate to output mesh
		for (int_max k = 0; k < AvailableCandidateList.GetLength(); ++k)
		{			
			if (this->Check_If_Candidate_is_Available(m_CandidateIndicatorList, AvailableCandidateList[k].GetIndex()) == true)
			{
				auto CandidateIndex_k = AvailableCandidateList[k].GetIndex();
				m_CandidateIndicatorList[CandidateIndex_k] = 1;
				SelectedCandidateAtRegionBoundary.Append(AvailableCandidateList[k]);	

				for (int_max n = 0; n < m_CandidateConflictTable_Overlap[CandidateIndex_k].GetLength(); ++n)
				{
					auto Index_n = m_CandidateConflictTable_Overlap[CandidateIndex_k][n];
					m_CandidateIndicatorList[Index_n] = 0;
				}
				for (int_max n = 0; n < m_CandidateConflictTable_TJunction[CandidateIndex_k].GetLength(); ++n)
				{
					auto Index_n = m_CandidateConflictTable_TJunction[CandidateIndex_k][n];
					m_CandidateIndicatorList[Index_n] = 0;
				}
			}
			else
			{
				m_CandidateIndicatorList[AvailableCandidateList[k].GetIndex()] = 0;
			}
		}
		...
		this->BuildMixedTriQuadMesh();
		//String TestDataPath = "C:/Research/Mesh/Remeshing/TestData/";
		SavePolygonMeshAsVTKFile(m_OutputMesh_Mixed, TestDataPath + "Region_Growing" + std::to_string(Counter)+ ".vtk");
		//--------------------------------------------
	}
}
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::SelectCandidate_RegionGrowing()
{
	auto CandidateIndexList_sort = m_CandidateScoreList.Sort("descend");
	m_CandidateIndicatorList.Resize(m_CandidateScoreList.GetLength());
	m_CandidateIndicatorList.Fill(-1);
	...
	while (Flag_OutputMesh_Completed == false)
	{
		DenseVector<FaceHandleType> AvailableCandidateHandleList;
		AvailableCandidateHandleList.SetCapacity(1000);
		for (int_max k = 0; k < SelectedCandidateAtRegionBoundary.GetLength(); ++k)
		{
			auto AdjacentCandidateHandleList = m_CandidateMesh.Face(SelectedCandidateAtRegionBoundary[k]).GetAdjacentFaceHandleList();
			auto AdjacentCandidateIndexList = this->ConvertHandleToIndex(AdjacentCandidateHandleList);
			auto ScoreList = m_CandidateScoreList.GetSubSet(AdjacentCandidateIndexList);
			auto IndexList_sort = ScoreList.Sort("descend");
			for (int_max n = 0; n < AdjacentCandidateHandleList.GetLength(); ++n)
			{
				auto CandidateHandle_n = AdjacentCandidateHandleList[IndexList_sort[n]];			
				if (this->Check_If_Candidate_is_Available(m_CandidateIndicatorList, CandidateHandle_n.GetIndex()) == true)
				{
					AvailableCandidateHandleList.Append(CandidateHandle_n);
				}
			}
		}

		AvailableCandidateHandleList = AvailableCandidateHandleList.GetSubSet(AvailableCandidateHandleList.FindUnique());

		if (AvailableCandidateHandleList.IsEmpty() == true)
		{
			Flag_OutputMesh_Completed = true;
			break;
		}
		...
		//----------------------------------
		{
			auto AvailableCandidateIndexList = this->ConvertHandleToIndex(AvailableCandidateHandleList);
			auto ScoreList = m_CandidateScoreList.GetSubSet(AvailableCandidateIndexList);
			auto IndexList_sort = ScoreList.Sort("descend");
			AvailableCandidateHandleList = AvailableCandidateHandleList.GetSubSet(IndexList_sort);
		}
		//----------------------------------
		...
		// add candidate to output mesh
		for (int_max k = 0; k < AvailableCandidateHandleList.GetLength(); ++k)
		{
			auto CandidateHandle_k = AvailableCandidateHandleList[k];
			auto CandidateIndex_k = CandidateHandle_k.GetIndex();
			if (this->Check_If_Candidate_is_Available(m_CandidateIndicatorList, CandidateIndex_k) == true)
			{				
				m_CandidateIndicatorList[CandidateIndex_k] = 1;
				SelectedCandidateAtRegionBoundary.Append(CandidateHandle_k);
			...
			{
				if (m_CandidateIndicatorList[CandidateIndex_k] == -1)
				{
					m_CandidateIndicatorList[CandidateIndex_k] = 0;
				}
			}
		}
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::SelectCandidate_CoverTriangle()
{
	auto CandidateIndexList_sort = m_CandidateScoreList.Sort("descend");
	m_CandidateIndicatorList.Resize(m_CandidateScoreList.GetLength());
	m_CandidateIndicatorList.Fill(-1);
	...
	for (int_max k = 0; k < m_BigTriangleCandidateCount; ++k)
	{
		const auto& Set = m_QuadCandidateIndexSet_At_BigTriangle[k];
		auto ScoreList = m_CandidateScoreList.GetSubSet(Set);
		auto IndexList_sort = ScoreList.Sort("descend");		
		for (int_max n = 0; n < Set.GetLength(); ++n)
		{
			auto CandidateIndex_n = Set[IndexList_sort[n]];
			if (this->Check_If_Candidate_is_Available(m_CandidateIndicatorList, CandidateIndex_n) == true)
			{
				m_CandidateIndicatorList[CandidateIndex_n] = 1;

				for (int_max m = 0; m < m_CandidateConflictTable_Overlap[CandidateIndex_n].GetLength(); ++m)
				{
					auto Index_m = m_CandidateConflictTable_Overlap[CandidateIndex_n][m];
					if (m_CandidateIndicatorList[Index_m] == 1)
					{
						int aaa = 1;
					}
					m_CandidateIndicatorList[Index_m] = 0;
				}
				for (int_max m = 0; m < m_CandidateConflictTable_TJunction[CandidateIndex_n].GetLength(); ++m)
				{
					auto Index_m = m_CandidateConflictTable_TJunction[CandidateIndex_n][m];
					if (m_CandidateIndicatorList[Index_m] == 1)
					{
						int aaa = 1;
					}
			...
			{
				if (m_CandidateIndicatorList[CandidateIndex_n] == -1)
				{
					m_CandidateIndicatorList[CandidateIndex_n] = 0;
				}
			}
		}
	}
...
template<typename ScalarType>
bool SurfaceRemesher6<ScalarType>::Check_If_Candidate_is_Available(const DenseVector<int_max>& CandidateIndicatorList, int_max CandidateIndex)
{
	if (CandidateIndicatorList[CandidateIndex] != -1)
	{
		return false;
	}
	...
	for (int_max n = 0; n < m_CandidateConflictTable_Overlap[CandidateIndex].GetLength(); ++n)
	{
		...
		if (CandidateIndicatorList[Index_n] == 1)
		{
			Flag_overlap = true;
			break;
		}
	...
	bool Flag_tjunction_self = false;
	for (int_max n = 0; n < m_CandidateConflictTable_TJunction[CandidateIndex].GetLength(); ++n)
	{
		...
		if (CandidateIndicatorList[Index_n] == 1)
		{
			Flag_tjunction_self = true;
			break;
		}
	}

	if (Flag_tjunction_self == true)
	{
		return false;
	}

	bool Flag_tjunction_other = false;
	for (int_max k = 0; k < CandidateIndicatorList.GetLength(); ++k)
	{
		if (CandidateIndicatorList[k] == 1)
		{
			for (int_max n = 0; n < m_CandidateConflictTable_TJunction[k].GetLength(); ++n)
			{
				if (CandidateIndex == m_CandidateConflictTable_TJunction[k][n])
				{
					Flag_tjunction_other = true;
					break;
				}
			}

			if (Flag_tjunction_other == true)
			{
				break;
			}
		}
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::BuildMixedTriQuadMesh()
{
	m_OutputMesh_Mixed.Clear();
	...
	int_max SelectedCandidateCount = 0;
...
template<typename ScalarType>
void SurfaceRemesher6<ScalarType>::BuildQuadMesh()
...
template<typename ScalarType>
DenseVector<int_max> SurfaceRemesher6<ScalarType>::ConvertHandleToIndex(const DenseVector<PointHandleType>& HandleList)
{
...
template<typename ScalarType>
DenseVector<int_max> SurfaceRemesher6<ScalarType>::ConvertHandleToIndex(const DenseVector<FaceHandleType>& HandleList)
{
	DenseVector<int_max> IndexList;
	IndexList.Resize(HandleList.GetLength());
	for (int_max k = 0; k < HandleList.GetLength(); ++k)
...
template<typename ScalarType>
ScalarType SurfaceRemesher6<ScalarType>::EvaluateQuad(const PolygonMesh<MeshAttributeType>& TargetMesh, PointHandleType PointH0, PointHandleType PointH1, PointHandleType PointH2, PointHandleType PointH3)
{
...
template<typename ScalarType>
ScalarType SurfaceRemesher6<ScalarType>::EvaluateQuad(const DenseVector<ScalarType, 3>& Point0, const DenseVector<ScalarType, 3>& Point1, const DenseVector<ScalarType, 3>& Point2, const DenseVector<ScalarType, 3>& Point3)
{
	//================
	//  P3 -  P2 
	//  |     |   
...
template<typename ScalarType>
ScalarType SurfaceRemesher6<ScalarType>::EvaluateTriangle(const DenseVector<ScalarType, 3>& Point0, const DenseVector<ScalarType, 3>& Point1, const DenseVector<ScalarType, 3>& Point2)
{
	//================
	//  P0  ---- P2 
	//     \    /   
