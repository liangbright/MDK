...
SurfaceMesh3DTo2DMapper1<ScalarType>::SurfaceMesh3DTo2DMapper1()
{
	this->Clear();
}
...
template<typename ScalarType>
void SurfaceMesh3DTo2DMapper1<ScalarType>::Clear()
{
	m_InputMesh.Clear();
...
template<typename ScalarType>
bool SurfaceMesh3DTo2DMapper1<ScalarType>::CheckInput()
{
	if (m_InputMesh.Check_If_DataStructure_is_Clean() == false)
	{
		MDK_Error("m_InputMesh DataStructure is NOT Clean @ SurfaceMesh3DTo2DMapper1::CheckInput()")
		return false;
	}
	return true;
}
...
template<typename ScalarType>
void SurfaceMesh3DTo2DMapper1<ScalarType>::Preprocess()
{
...
template<typename ScalarType>
void SurfaceMesh3DTo2DMapper1<ScalarType>::Update()
{
...
template<typename ScalarType>
void SurfaceMesh3DTo2DMapper1<ScalarType>::ApplyMeanValueBasedParameterization()
{
	auto PointCount_boundary = m_BoundaryPointHandleList.GetLength();
...
template<typename ScalarType>
ObjectArray<SparseVector<ScalarType>> SurfaceMesh3DTo2DMapper1<ScalarType>::ComputeWeightMatrix_MeanValue(const TriangleMesh<MeshAttributeType>& TargetMesh)
{
	auto PointCount_boundary = m_BoundaryPointHandleList.GetLength();
	auto PointCount_inner = m_InnerPointHandleList.GetLength();
...
template<typename ScalarType>
ScalarType SurfaceMesh3DTo2DMapper1<ScalarType>::ComputeWeight_MeanValue(const TriangleMesh<MeshAttributeType>& TargetMesh, PointHandleType PointH0, PointHandleType PointH1)
{
...
template<typename ScalarType>
void SurfaceMesh3DTo2DMapper1<ScalarType>::ApplyStretchMinimizationBasedParameterization()
{
	auto PointCount_boundary = m_BoundaryPointHandleList.GetLength();
	auto PointCount_inner = m_InnerPointHandleList.GetLength();
	...
	for (int_max Iter = 0; Iter < MaxIter; ++Iter)
	{
		...
		auto StretchList = this->ComputeLocalStretch_StretchMinimization(m_InputMesh, m_OutputMesh);		
		for (int_max k = 0; k < PointCount_inner; ++k)
		{
			...
			{
				auto Stretch = StretchList[AdjPointIndexList[n]];
...
template<typename ScalarType>
DenseVector<ScalarType> SurfaceMesh3DTo2DMapper1<ScalarType>::ComputeLocalStretch_StretchMinimization(const TriangleMesh<MeshAttributeType>& InputMesh, const TriangleMesh<MeshAttributeType>& OutputMesh)
{
	...
	for (int_max k = 0; k < FaceCount; ++k)
	{
		...
		auto PointHandleList_input = InputMesh.Face(FaceHandle).GetPointHandleList();
		auto PointPosition_input = InputMesh.GetPointPosition(PointHandleList_input);
		auto PointHandleList_output = OutputMesh.Face(FaceHandle).GetPointHandleList();
		auto PointPosition_output = OutputMesh.GetPointPosition(PointHandleList_output);
		...
		*/
		...
		auto q1 = InputMesh.GetPointPosition(PointHandleList_input[0]);
		auto q2 = InputMesh.GetPointPosition(PointHandleList_input[1]);
		auto q3 = InputMesh.GetPointPosition(PointHandleList_input[2]);
...
template<typename ScalarType>
DenseMatrix<ScalarType> SurfaceMesh3DTo2DMapper1<ScalarType>::ComputeUV_Given_WeightMatrix(const ObjectArray<SparseVector<ScalarType>>& WeightMatrix)
{
...
template<typename ScalarType>
DenseVector<typename SurfaceMesh3DTo2DMapper1<ScalarType>::PointHandleType> 
SurfaceMesh3DTo2DMapper1<ScalarType>::ConvertIndexToHandle(const DenseVector<int_max>& IndexList)
{
