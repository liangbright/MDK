...
#include "mdkConvolutionDenseImageFilter3D.h"

using namespace mdk;

void test_GaussianFilter3D()
{
	...
	Save3DScalarImageAsJsonDataFile(InputImage, TestDataPath + "test_GaussianFilter3D_InputImage.json");

	//InputImage.SetSpacing(1.0, 1.0, 1.0);
	//InputImage.SetSize(100, 100, 100);

	ConvolutionDenseImageFilter3D<double> imfilter;
    imfilter.SetInputImage(&InputImage);
	imfilter.SetBoundaryOptionAsReplicate();	
	//imfilter.CreateGaussianMask(9, 9, 6, 1.8);
	imfilter.CreateGaussianMask(InputImage.GetInfo(), 9, 9, 9, 1);
    ...
    Save3DScalarImageAsJsonDataFile(OutputImage, TestDataPath + "test_GaussianFilter3D_OutputImage.json");

    std::system("pause");
}

void test_LoGFilter3D()
{
	String TestDataPath = "C:/Research/MDK/MDK_Build/Test/Test_ImageProcessing/Test_ImageFilter/Test_ConvolutionImageFilter3D/TestData/";
	...
	imfilter.SetInputImage(&InputImage);
	imfilter.SetBoundaryOptionAsReplicate();
	imfilter.CreateLaplacianOfGaussianMask(InputImage.GetInfo(), 1.5, 2);
	imfilter.SetMaxThreadCount(8);
	...
	Save3DScalarImageAsJsonDataFile(OutputImage, TestDataPath + "test_LoGFilter3D_OutputImage.json");
