...
bool KNNBasisSelectionBasedShapeDictionaryBuilder<ScalarType>::CheckInput()
{
    ...
    {
        MDK_Error("Input.TrainingShapeData is nullptr @ KNNBasisSelectionBasedShapeDictionaryBuilder::CheckInput()")
        return false;
    }
	...
	{
		MDK_Error("BatchSize is out of range @ KNNBasisSelectionBasedShapeDictionaryBuilder::CheckInput()")
		return false;
	}
	
	if (Input.Parameter.MaxNeighborCount <= 0)
	{
		MDK_Error("MaxNeighborCount <= 0 @ KNNBasisSelectionBasedShapeDictionaryBuilder::CheckInput()")
		return false;
	}
...
void KNNBasisSelectionBasedShapeDictionaryBuilder<ScalarType>::Update()
{
	...
	{
		TotalBasisExperience_init = OutputDictionary.BasisExperience().Sum()*Input.Parameter.ExperienceDiscountFactor;
	}
...
{
	ScalarType TotalGain = BasisExperience.Sum() - TotalBasisExperience_init;
	if (TotalGain > TotalDataCount)
	{
		BasisExperience *= ScalarType(TotalDataCount) / TotalGain;	
	}
	for (int_max k = 0; k < BasisExperience.GetElementCount(); ++k)
	{
		if (BasisExperience[k] < 1)
...
BuildDictionaryInMiniBatch(const ShapeDictionary<ScalarType>& Dictionary_init, const ObjectArray<DenseMatrix<ScalarType>>& ShapeData)
{
	...
	DenseVector<ScalarType> ShapeExperience_init;
	ShapeExperience_init.Resize(TotalShapeCount);
	ShapeExperience_init.Fill(1);
	for (int_max k = 0; k < BasisCount_init; ++k)
	{
		ShapeExperience_init[k] = Dictionary_init.BasisExperience()[k];
	}

    auto ShapeIndexList_Basis = this->SelectBasis(BasisCount_desired, ShapeSimilarityMatrix, ShapeExperience_init);
...
template<typename ScalarType>
DenseVector<int_max> KNNBasisSelectionBasedShapeDictionaryBuilder<ScalarType>::SelectBasis(const int_max BasisCount_desired, const DenseMatrix<ScalarType>& ShapeSimilarityMatrix, const DenseVector<ScalarType>& ShapeExperience_init)
{
	//---------------------------------------------------------------------
	...
	DenseVector<ScalarType> ShapeExperience;
	ShapeExperience = ShapeExperience_init;
	for (int_max k = 0; k < TotalShapeCount; ++k)
	{		
		auto SimilarityList = ShapeSimilarityMatrix.RefCol(k);
		DenseVector<int_max> ShapeIndexList_KNN = FindKNNBySimilarityList(SimilarityList, Input.Parameter.MaxNeighborCount);
		DenseVector<ScalarType> MembershipList_KNN = SimilarityList.GetSubMatrix(ShapeIndexList_KNN);
		MembershipList_KNN /= (MembershipList_KNN.Sum() + EPS);
		if (ShapeIndexList_KNN.GetLength() > 0)
		{
			for (int_max n = 0; n < ShapeIndexList_KNN.GetLength(); ++n)
			{
				int_max ShapeIndex_n = ShapeIndexList_KNN[n];
				ShapeExperience[ShapeIndex_n] += MembershipList_KNN[n];
			}
		}
	}
	auto ShapeIndexList_sort = ShapeExperience.Sort("descend");
...
template<typename ScalarType>
DenseVector<int_max> KNNBasisSelectionBasedShapeDictionaryBuilder<ScalarType>::SelectBasis_By_SimilarityThreshold(const DenseVector<int_max>& ShapeIndexList_sort, const DenseMatrix<ScalarType>& ShapeSimilarityMatrix, ScalarType SimilarityThreshold)
{
                                            ...
                                            const ShapeDictionary<ScalarType>& Dictionary_init)
{
	...
	for (int_max k = 0; k < BasisCount; ++k)
	{
		...
		SimilarityList = SimilarityList.GetSubMatrix(ShapeIndexList_Basis);
		DenseVector<int_max> KNNBasisIndexList = FindKNNBySimilarityList(SimilarityList, Input.Parameter.MaxNeighborCount);
		DenseVector<ScalarType> KNNBasisMembershipList = SimilarityList.GetSubMatrix(KNNBasisIndexList);
		KNNBasisMembershipList /= (KNNBasisMembershipList.Sum() + EPS);
		if (KNNBasisIndexList.GetLength() > 0)
		{
			for (int_max n = 0; n < KNNBasisIndexList.GetLength(); ++n)
			{
				int_max BasisIndex_n = KNNBasisIndexList[n];
				BasisExperience[BasisIndex_n] += KNNBasisMembershipList[n];
			}
		}
	}
