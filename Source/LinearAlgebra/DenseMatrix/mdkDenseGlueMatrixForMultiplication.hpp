...
template<typename ElementType>
inline
mdkDenseMatrix<ElementType> mdkDenseGlueMatrixForMultiplication<ElementType>::CreateMatrix() const
{
...
template<typename ElementType>
inline 
bool mdkDenseGlueMatrixForMultiplication<ElementType>::CreateMatrix(mdkDenseMatrix<ElementType>& OutputMatrix) const
{
    if (m_RowNumber != OutputMatrix.GetRowNumber() || m_ColNumber != OutputMatrix.GetColNumber())
    {
        ...
        {
            mdkError << "Size does not match @ mdkDenseGlueMatrixForMultiplication::CreateMatrix(OutputMatrix)" << '\n';
            return false;
        }
    }
    ...
    if (MatrixNumber == 0)
    {
        mdkError << "MatrixNumber is zero @ mdkDenseGlueMatrixForMultiplication::CreateMatrix(OutputMatrix)" << '\n';
        return false;
    }
    ...
    if (MatrixNumber == 1)
    {
        ...
        {
            mdkError << "MatrixNumber is 1 and  m_Is_m_Element_Coef_Equal_to_One = true @ mdkDenseGlueMatrixForMultiplication::CreateMatrix(OutputMatrix)" << '\n';
