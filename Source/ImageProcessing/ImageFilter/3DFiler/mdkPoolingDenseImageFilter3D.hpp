...
template<typename InputPixelType, typename OutputPixelType, typename ScalarType>
void PoolingDenseImageFilter3D<InputPixelType, OutputPixelType, ScalarType>::SetRadius(double RadiusX, double RadiusY, double RadiusZ)
{
	m_Radius[0] = RadiusX;
	m_Radius[1] = RadiusY;
	m_Radius[2] = RadiusZ;
...
bool PoolingDenseImageFilter3D<InputPixelType, OutputPixelType, ScalarType>::CheckInput()
{
	...
	{
		MDK_Error("m_MaxThreadCount <= 0 @ PoolingDenseImageFilter3D::CheckInput()")
		return false;
	}

	if (m_Radius[0] <= 0 || m_Radius[1] <= 0 || m_Radius[2] <= 0)
	{
		MDK_Error("m_Radius <= 0 @ PoolingDenseImageFilter3D::CheckInput()")
		return false;
	}
