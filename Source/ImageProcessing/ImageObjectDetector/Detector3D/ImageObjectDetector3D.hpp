#ifndef __mdkImageObjectDetector3D_hpp
#define __mdkImageObjectDetector3D_hpp

namespace mdk
{
template<typename InputPixelType, typename ScalarType>
ImageObjectDetector3D<InputPixelType, ScalarType>::ImageObjectDetector3D()
{
	this->Clear();
}
...
template<typename InputPixelType, typename ScalarType>
ImageObjectDetector3D<InputPixelType, ScalarType>::Clear()
{
...
template<typename InputPixelType, typename ScalarType>
ImageObjectDetector3D<InputPixelType, ScalarType>::ClearOutput()
{
	this->ImageFilter3D::ClearOutput();
	m_Output.Clear();
...
template<typename InputPixelType, typename ScalarType>
ObjectArray<DenseMatrix<double>>& ImageObjectDetector3D<InputPixelType, ScalarType>::MaskList()
{
	return m_MaskList;
}
...
template<typename InputPixelType, typename ScalarType>
const ObjectArray<DenseMatrix<double>>& ImageObjectDetector3D<InputPixelType, ScalarType>::MaskList() const
{
	return m_MaskList;
}
...
template<typename InputPixelType, typename ScalarType>
void ImageObjectDetector3D<InputPixelType, ScalarType>::Use3DIndexInMask()
{
...
template<typename InputPixelType, typename ScalarType>
void ImageObjectDetector3D<InputPixelType, ScalarType>::Use3DPhysicalPositionInMask()
{
	m_Flag_3DPositionInMask = 1;
}
...
template<typename InputPixelType, typename ScalarType>
void ImageObjectDetector3D<InputPixelType, ScalarType>::ComputeRegionOfNOBoundCheck_3DIndex()
{
	...
	for (int_max i = 0; i < MaskNumber; ++i)
	{
		...
		for (int_max k = 0; k < m_MaskList[i].PointSet.GetElementNumber(); ++k)
		{
			for (int_max j = 0; j < m_MaskList[i].PointSet[i].GetColNumber(); ++j)
			{
				auto temp_x = m_MaskList[i].PointSet[i](0, j);
...
template<typename InputPixelType, typename ScalarType>
void ImageObjectDetector3D<InputPixelType, ScalarType>::ComputeRegionOfNOBoundCheck_3DPosition()
{
	if (m_MaskList.IsEmpty() == true)
	{
		m_NOBoundCheckRegionList_3DPosition.Clear();
...
template<typename InputPixelType, typename ScalarType>
bool ImageObjectDetector3D<InputPixelType, ScalarType>::Preprocess()
{
	...
	m_Output.FastResize(m_TotalOutputPixelNumber);

	for (int_max k = 0; k < m_MaskList.GetElementNumber(); ++k)
	{
		m_MaskList[k].Flag_3DPosition = m_Flag_3DPositionInMask;
	}
	...
	{
		MDK_Error("m_Flag_3DPositionInMask is -1 @ ImageObjectDetector3D::Preprocess()")
		return false;
	}
...
template<typename InputPixelType, typename ScalarType>
bool ImageObjectDetector3D<InputPixelType, ScalarType>::WhetherToCheckBoundAtMaskOrigin_3DIndex(double x, double y, double z, int_max MaskIndex)
{
...
template<typename InputPixelType, typename ScalarType>
bool ImageObjectDetector3D<InputPixelType, ScalarType>::WhetherToCheckBoundAtMaskOrigin_3DPosition(double x, double y, double z, int_max MaskIndex)
{
	bool WhetherToCheck = false;
...
template<typename InputPixelType, typename ScalarType>
inline
void ImageObjectDetector3D<InputPixelType, ScalarType>::
OutputFunction(int_max OutputPixelIndex, OutputPixelType& OutputPixel, int_max ThreadIndex)
{
	m_Output[OutputPixelIndex] = std::move(OutputPixel);
}
