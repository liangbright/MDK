...
public: 
	...
	int_max& source_index(){return m_source_index;};
...
private: 
	...
	int_max m_source_index;			//closest source index
...
public:
	GeodesicAlgorithmSubdivision(geodesic::Mesh* mesh = NULL, 
								 int_max subdivision_level = 0):
		GeodesicAlgorithmGraphBase<Node>(mesh)
	{
		m_type = SUBDIVISION;

		m_nodes.reserve(mesh->vertices().size());
        for (int_max i = 0; i<int_max(mesh->vertices().size()); ++i)
		{
	...
	int_max subdivision_level(){return m_subdivision_level;};

	void set_subdivision_level(int_max subdivision_level)
	{
        ...
        for (int_max i = 0; i<int_max(m_mesh->edges().size()); ++i)
		{
			edge_pointer e = &m_mesh->edges()[i];
			for(int_max i=0; i<subdivision_level; ++i)
			{
...
protected:
	...
	int_max node_indexx(edge_pointer e)
	{
...
private:
	...
	int_max m_subdivision_level;	//when level is equal to 1, this algorithm corresponds to the Dijkstra algorithm
};
													 ...
													 double threshold_distance)
{
	...
	else if(p->type() == EDGE)
	{
		...
		int_max node_index = node_indexx(e);
		for(int_max i=0; i<m_subdivision_level; ++i)
		{
...
inline void GeodesicAlgorithmSubdivision::list_nodes_visible_from_source(MeshElementBase* p, std::vector<node_pointer>& storage)
{
	assert(p->type() != UNDEFINED_POINT);
																       ...
																       double threshold_distance)
{
	...
	int_max index = distances.size();
	distances.resize(storage.size());
    for (; index<int_max(storage.size()); ++index)
	{	
