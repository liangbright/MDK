...
public:
	...
	void initialize_mesh_data(int_max num_vertices,
							  Points& p, 
							  int_max num_faces,
	...
	int_max closest_vertices(SurfacePoint* p,  std::vector<vertex_pointer>* storage = NULL);		//list vertices closest to the point
...
private:
	...
	void_pointer allocate_pointers(int_max n) 
	{
...
inline void Mesh::Clear()
{
    ...
    m_pointer_allocator.reset(0, 0);
}

inline int_max Mesh::closest_vertices(SurfacePoint* p, 
										  std::vector<vertex_pointer>* storage)
{
	...
	else if(p->type() == EDGE)		//for edge include all 4 adjacent vertices
	{
		...
		if(storage)
		{
			...
			for(int_max i = 0; i < edge->adjacent_faces().size(); ++i)
			{
...
{
	assert(p.size() % 3 == 0);
	int_max const num_vertices = p.size() / 3;
	assert(tri.size() % 3 == 0);
	int_max const num_faces = tri.size() / 3; 

	initialize_mesh_data(num_vertices, p, num_faces, tri);
}

template<class Points, class Faces>
void Mesh::initialize_mesh_data(int_max num_vertices,
								Points& p, 
								int_max num_faces,
								Faces& tri)
{
	int_max const approximate_number_of_internal_pointers = (num_vertices + num_faces)*4;
	int_max const max_number_of_pointer_blocks = 100; 
	m_pointer_allocator.reset(approximate_number_of_internal_pointers, 
							  max_number_of_pointer_blocks);

	m_vertices.resize(num_vertices);
	for(int_max i=0; i<num_vertices; ++i)		//copy coordinates to vertices
	{
	...
	for(int_max i=0; i<num_faces; ++i)		//copy adjacent vertices to polygons/faces
	{
		Face& f = m_faces[i];
		f.id() = i;
		...
		{
			int_max vertex_index = tri[shift + j];
...
inline void Mesh::build_adjacencies()
{
	...
	std::vector<int_max> count(m_vertices.size());	//count adjacent vertices
	for(int_max i=0; i< int_max(m_faces.size()); ++i)
	{
		Face& f = m_faces[i];
		for(int_max j=0; j<3; ++j)
		{
			int_max vertex_id = f.adjacent_vertices()[j]->id();
			assert(vertex_id < int_max(m_vertices.size()));
			count[vertex_id]++;
		}
	}

    for (int_max i = 0; i< int_max(m_vertices.size()); ++i)		//reserve space
	{
		Vertex& v = m_vertices[i];
		int_max num_adjacent_faces = count[i];
    ...
    for (int_max i = 0; i<int_max(m_faces.size()); ++i)
	{
		...
		for(int_max j=0; j<3; ++j)
		{
			...
			int_max vertex_id_1 = f.adjacent_vertices()[j]->id();
			int_max vertex_id_2 = f.adjacent_vertices()[(j+1) % 3]->id();
	...
	int_max number_of_edges = 1;
    for (int_max i = 1; i<int_max(half_edges.size()); ++i)
	{
		...
		{
            if (i<int_max(half_edges.size()) - 1)		//sanity check: there should be at most two equal half-edges
	...
	int_max edge_id = 0;
    for (int_max i = 0; i<int_max(half_edges.size());)
	{
	...
	std::fill(count.begin(), count.end(), 0);
    for (int_max i = 0; i<int_max(m_edges.size()); ++i)
	{
		Edge& e = m_edges[i];
    ...
    for (int_max i = 0; i<int_max(m_vertices.size()); ++i)
	{
	...
	{
		Edge& e = m_edges[i];
        for (int_max j = 0; j<int_max(e.adjacent_faces().size()); ++j)
		{
    ...
    for (int_max i = 0; i<int_max(m_faces.size()); ++i)
	{
		...
		double abc[3];		
		double sum = 0;
		for(int_max j=0; j<3; ++j)		//compute angle adjacent to the vertex j
		{
...
inline bool Mesh::verify()		//verifies connectivity of the mesh and prints some debug info
{
	...
	{
		using namespace mdk;
		MDK_Error("some vertice is not mentioned, this error may be igored @ geodesic");
	}
    ...
    {
        using namespace mdk;
        MDK_Error("mesh has more than one connected component, this error may be igored @ geodesic");
    }
	...
	int_max total_boundary_edges = 0;
...
{
	point->set(data);
	int_max type = (int_max) data[3];
	int_max id = (int_max) data[4];
