...
public:
	...
	{
		m_type = DIJKSTRA;
		for(int_max i=0; i<m_nodes.size(); ++i)
		{
	...
	virtual int_max best_source(SurfacePoint& point,			//quickly find what source this point belongs to and what is the distance to this source
...
inline int_max GeodesicAlgorithmDijkstraAlternative::best_source(SurfacePoint& point,			//quickly find what source this point belongs to and what is the distance to this source
																	   double& best_source_distance)
{
	...
	else
	{
		...
		for(int_max i=0; i<possible_vertices.size(); ++i)
		{
															...
															std::vector<SurfacePoint>* stop_points)   //or after ensuring that all the stop_points are covered
{
	...
	for(int_max i=0; i<m_sources.size(); ++i)
	{
		SurfacePoint* s = &m_sources[i];
		m_mesh->closest_vertices(s, &visible_vertices);
		for(int_max j=0; j<visible_vertices.size(); ++j)
		{
	...
	for(int_max i=0; i<m_nodes.size(); ++i)		//initialize the queue
	{
	...
	while(!m_queue.empty())					//main cycle
	{
		...
		for(int_max i=0; i<v->adjacent_edges().size(); ++i)		//update all the adgecent vertices
		{
...
inline bool GeodesicAlgorithmDijkstraAlternative::check_stop_conditions(int_max& index)
{
