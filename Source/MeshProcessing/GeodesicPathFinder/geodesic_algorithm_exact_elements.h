...
protected:
	...
	int_max m_source_index;			//the source it belongs to
...
{
public:
	int_max index(){return m_index;};

	void initialize(SurfacePoint& p, int_max index)
	{
...
public:
	...
	void initialize(std::vector<SurfacePoint>& sources)	//we initialize the sources by copie
	{
        ...
        for (int_max i = 0; i<int_max(sources.size()); ++i)
		{
	...
	SurfacePointWithIndex& operator[](int_max i)
	{
        assert(i < int_max(size()));
		return *(begin() + i);
	}
