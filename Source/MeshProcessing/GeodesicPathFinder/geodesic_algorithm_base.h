...
public:
	...
	virtual int_max best_source(SurfacePoint& point,			//after propagation step is done, quickly find what source this point belongs to and what is the distance to this source
...
inline double length(std::vector<SurfacePoint>& path)
{
	...
	if(!path.empty())
	{
        for (int_max i = 0; i<int_max(path.size()) - 1; ++i)
		{
											...
											std::vector<std::vector<SurfacePoint> >& paths) //lazy people can find geodesic paths with one function call
{
    ...
    for (int_max i = 0; i<int_max(paths.size()); ++i)
	{
														...
														double stop_distance)
{
    ...
    for (int_max i = 0; i < int_max(stop_points->size()); ++i)
	{
        ...
        for (int_max j = 0; j < int_max(possible_vertices.size()); ++j)
		{
