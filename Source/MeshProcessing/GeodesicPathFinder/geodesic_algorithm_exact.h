...
public:
		...
		m_edge_interval_lists(mesh->edges().size())
	{
        ...
        for (int_max i = 0; i<int_max(m_edge_interval_lists.size()); ++i)
		{
	...
	int_max best_source(SurfacePoint& point,			//quickly find what source this point belongs to and what is the distance to this source
...
private:
	...
	void update_list_and_queue(list_pointer list,
							   ...
							   int_max num_candidates);

	int_max compute_propagated_parameters(double pseudo_x, 
											double pseudo_y, 
	...
	int_max intersect_intervals(interval_pointer zero, 
	...
	interval_pointer best_first_interval(SurfacePoint& point, 
										...
										int_max& best_source_index);

	bool check_stop_conditions(int_max& index);
	...
	int_max m_queue_max_size;			//used for statistics
	int_max m_iterations;			//used for statistics
															   ...
															   double& best_interval_position)
{
    ...
    for (int_max i = 0; i<int_max(storage.size()); ++i)
	{
															 ...
															 std::vector<edge_pointer>& storage)
{
	...
	if(point.type() == VERTEX)
	{
		...
		for(int_max i=0; i<v->adjacent_faces().size(); ++i)
		{
	...
	else if(point.type() == EDGE)
	{
		...
		for(int_max i=0; i<e->adjacent_faces().size(); ++i)
		{
...
inline long GeodesicAlgorithmExact::visible_from_source(SurfacePoint& point)	//negative if not visible
{
	...
	{
		vertex_pointer v = static_cast<vertex_pointer>(point.base_element());
		for(int_max i=0; i<v->adjacent_edges().size(); ++i)
		{
...
inline int_max GeodesicAlgorithmExact::intersect_intervals(interval_pointer zero, 
															   IntervalWithStop* one)			//intersecting two intervals with up to three intervals in the end
{
...
inline void GeodesicAlgorithmExact::initialize_propagation_data()
{
    ...
    for (int_max i = 0; i<int_max(m_sources.size()); ++i)		//for all edges adjacent to the starting vertex			
	{
        ...
        for (int_max j = 0; j<int_max(edges_visible_from_source.size()); ++j)
		{
									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	while(!m_queue.empty())
	{
		m_queue_max_size = std::max<int_max>(m_queue.size(), m_queue_max_size);

		int_max const check_period = 10;
		...
		for(int_max i=0; i<edge->adjacent_faces().size(); ++i)		//two possible faces to propagate
		{
			...
			int_max num_propagated = compute_propagated_parameters(min_interval->pseudo_x(), 
...
/*	for(int_max i=0; i<m_edge_interval_lists.size(); ++i)
	{
...
inline bool GeodesicAlgorithmExact::check_stop_conditions(int_max& index)
{
    ...
    while (index < int_max(m_stop_vertices.size()))
	{
												...
												int_max num_candidates)
{
	...
	for(int_max i=0; i<num_candidates; ++i)				//for all new intervals
	{
		...
		{
			int_max const N = intersect_intervals(p, q);								//interset two intervals
			...
			for(int_max j=1; j<N; ++j)				//no memory is needed for the first one
			{
			...
			for(int_max j=0; j<N; ++j)								//find "min" and add the intervals to the queue
			{
				if(j==N-1 && map[j]==NEW)
				{
					previous = i_new[j];
...
inline int_max GeodesicAlgorithmExact::compute_propagated_parameters(double pseudo_x, 
																		double pseudo_y, 
																		double d,		//parameters of the interval
																		double begin, 
																		double end,		//start/end of the interval
																	...
																	interval_pointer source_interval)	//up to two candidates
{
	...
	{
		for(int_max i=0; i<num_candidates; ++i)
		{
...
inline int_max GeodesicAlgorithmExact::best_source(SurfacePoint& point,			//quickly find what source this point belongs to and what is the distance to this source
													   double& best_source_distance)
{
	double best_interval_position; 
	int_max best_source_index;
...
{					
	...
	double best_total_distance; 
	double best_interval_position;
	int_max source_index = std::numeric_limits<int_max>::max();
