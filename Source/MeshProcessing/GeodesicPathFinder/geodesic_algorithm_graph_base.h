...
protected:
	int_max node_index(vertex_pointer v)		//gives index of the node that corresponds to this vertex
	{
		return v->id();
	};
	...
	node_pointer best_first_node(SurfacePoint& point, double& best_total_distance)
	{
		...
		else
		{
            ...
            for (int_max i = 0; i<int_max(possible_nodes.size()); ++i)
			{
	...
	bool check_stop_conditions(int_max& index);		//check when propagation should stop
												 ...
												 std::vector<SurfacePoint>* stop_points) //or after ensuring that all the stop_points are covered
{
    ...
    for (int_max i = 0; i<int_max(m_nodes.size()); ++i)
	{
		m_nodes[i].clear();
	}
    ...
    for (int_max i = 0; i<int_max(m_sources.size()); ++i)
	{
		SurfacePoint* source = &m_sources[i];
		list_nodes_visible_from_source(source->base_element(), 
									   visible_nodes);			

		for(int_max j=0; j< int_max(visible_nodes.size()); ++j)
		{
    ...
    for (int_max i = 0; i<int_max(m_nodes.size()); ++i)		//initialize the queue
	{
	...
	while(!m_queue.empty())					//main cycle
	{
        ...
        for (int_max i = 0; i<int_max(visible_nodes.size()); ++i)		//update all the adjacent vertices
		{
...
template<class Node>
inline bool GeodesicAlgorithmGraphBase<Node>::check_stop_conditions(int_max& index)
{
...
template<class Node>
inline int_max GeodesicAlgorithmGraphBase<Node>::best_source(SurfacePoint& point,			//quickly find what source this point belongs to and what is the distance to this source
																 double& best_source_distance)
{
