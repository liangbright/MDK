...
public:
	...
	SimlpeMemoryAllocator(int_max block_size = 0, 
						  int_max max_number_of_blocks = 0)
	{
		reset(block_size, 
	...
	void reset(int_max block_size, 
			   int_max max_number_of_blocks)
	{
	...
	pointer allocate(int_max const n)		//allocate n units
	{
...
private:
	...
	int_max m_block_size;				//size of a single block
	int_max m_max_number_of_blocks;		//maximum allowed number of blocks
	int_max m_current_position;			//first unused element inside the current block
};
...
public:
	...
	MemoryAllocator(int_max block_size = 1024, 
				    int_max max_number_of_blocks = 1024)
	{
		reset(block_size, 
			  max_number_of_blocks);
	};
...
public:
	...
	template<class T>
	T* allocate(int_max n)
	{
		double wanted = n*sizeof(T);
		if(wanted > m_num_bytes)
		{
			int_max new_size = (int_max) ceil(wanted / (double)sizeof(double));
	...
	int_max capacity()
	{
		return (int_max)floor((double)m_num_bytes/(double)sizeof(T));
	};
