...
template<typename ScalarType>
inline String Mesh<ScalarType>::GetCellName(int_max CellIndex) const
{
	if (this->IsValidCellIndex(CellIndex) == true)
	{
		return m_MeshData->CellList[CellIndex].GetName();
	}
	else
	{
		String EmptyName;
...
template<typename ScalarType>
inline StdObjectVector<String> Mesh<ScalarType>::GetCellName(const DenseVector<int_max>& CellIndexList) const
{
	StdObjectVector<String> NameList;
	NameList.Resize(CellIndexList.GetLength());
	for (int_max k = 0; k < CellIndexList.GetLength(); ++k)
	{
		NameList[k] = this->GetCellName(CellIndexList[k]);
	}
	return NameList;
}

//----------- change Point/Edge/Face Name -----------------------------------------------------------//
template<typename ScalarType>
inline void Mesh<ScalarType>::ChangePointName(const String& Name_old, const String& Name_new)
{
	auto PointIndex = this->GetPointIndexByName(Name_old);
	if (this->IsValidPointIndex(PointIndex) == true)
	{
		m_MeshData->PointList[PointIndex].SetName(Name_new);
	}
}

template<typename ScalarType>
inline void Mesh<ScalarType>::ChangeEdgeName(const String& Name_old, const String& Name_new)
{
	auto EdgeIndex = this->GetEdgeIndexByName(Name_old);
	if (this->IsValidEdgeIndex(EdgeIndex) == true)
	{
		m_MeshData->EdgeList[EdgeIndex].SetName(Name_new);
	}
}

template<typename ScalarType>
inline void Mesh<ScalarType>::ChangeFaceName(const String& Name_old, const String& Name_new)
{
	auto FaceIndex = this->GetFaceIndexByName(Name_old);
	if (this->IsValidFaceIndex(FaceIndex) == true)
	{
		m_MeshData->FaceList[FaceIndex].SetName(Name_new);
	}
}

template<typename ScalarType>
inline void Mesh<ScalarType>::ChangeCellName(const String& Name_old, const String& Name_new)
{
	auto CellIndex = this->GetCellIndexByName(Name_old);
	if (this->IsValidCellIndex(CellIndex) == true)
	{
		m_MeshData->CellList[CellIndex].SetName(Name_new);
	}
}
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangePointDataSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->PointDataSet.GetLength())
	{
		MDK_Error("PointDataSet " << Index << " NOT exist @ Mesh::ChangePointDataSetName(...)")
		return;
	}
	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Input Name is empty @ Mesh::ChangePointDataSetName(...)")
		return;
	}
	auto Index_test = this->GetPointDataSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Can not change Name @ Mesh::ChangePointDataSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->PointDataSetName[Index];
	m_MeshData->PointDataSetName[Index] = Name_new;
	auto it = m_MeshData->Map_PointDataSet_Name_to_Index.find(Name_old);
	m_MeshData->Map_PointDataSet_Name_to_Index.erase(it);
	m_MeshData->Map_PointDataSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangePointDataSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetPointDataSetIndex(Name_old);
	this->ChangePointDataSetName(Index, Name_new);
}
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangeEdgeDataSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->EdgeDataSet.GetLength())
	{
		MDK_Error("EdgeDataSet " << Index << " NOT exist @ Mesh::ChangeEdgeDataSetName(...)")
		return;
	}
	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Input Name is empty @ Mesh::GetEdgeDataSetIndex(...)")
		return;
	}
	auto Index_test = this->GetEdgeDataSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Can not change Name @ Mesh::ChangeEdgeDataSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->EdgeDataSetName[Index];
	m_MeshData->EdgeDataSetName[Index] = Name_new;
	auto it = m_MeshData->Map_EdgeDataSet_Name_to_Index.find(Name_old);
	m_MeshData->Map_EdgeDataSet_Name_to_Index.erase(it);
	m_MeshData->Map_EdgeDataSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangeEdgeDataSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetEdgeDataSetIndex(Name_old);
	this->ChangeEdgeDataSetName(Index, Name_new);
}
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangeFaceDataSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->FaceDataSet.GetLength())
	{
		MDK_Error("FaceDataSet " << Index << " NOT exist @ Mesh::ChangeFaceDataSetName(...)")
		return;
	}
	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Input Name is empty @ Mesh::ChangeFaceDataSetName(...)")
		return;
	}
	auto Index_test = this->GetFaceDataSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Can not change Name @ Mesh::ChangeFaceDataSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->FaceDataSetName[Index];
	m_MeshData->FaceDataSetName[Index] = Name_new;
	auto it = m_MeshData->MapFaceDataSet_Name_to_Index.find(Name_old);
	m_MeshData->MapFaceDataSet_Name_to_Index.erase(it);
	m_MeshData->Map_FaceDataSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangeFaceDataSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetFaceDataSetIndex(Name_old);
	this->ChangeFaceDataSetName(Index, Name_new);
}
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangeCellDataSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->CellDataSet.GetLength())
	{
		MDK_Error("CellDataSet " << Index << " NOT exist @ Mesh::ChangeCellDataSetName(...)")
		return;
	}
	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Input Name is empty @ Mesh::ChangeCellDataSetName(...)")
		return;
	}
	auto Index_test = this->GetCellDataSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Can not change Name @ Mesh::ChangeCellDataSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->CellDataSetName[Index];
	m_MeshData->CellDataSetName[Index] = Name_new;
	auto it = m_MeshData->Map_CellDataSet_Name_to_Index.find(Name_old);
	m_MeshData->Map_CellDataSet_Name_to_Index.erase(it);
	m_MeshData->Map_CellDataSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangeCellDataSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetCellDataSetIndex(Name_old);
	this->ChangeCellDataSetName(Index, Name_new);
}
...
int_max Mesh<ScalarType>::AddPointSet(const String& Name, DenseVector<int_max> PointIndexList)
{
	...
	{		
		MDK_Error("PointSet " << Name << " has already been added @ Mesh::AddPointSet(...)")
		return -1;
	}
	else
	{
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangePointSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->PointSet.GetLength())
	{
		MDK_Error("Index " << Index << " NOT exist @ Mesh::ChangePointSetName(...)")
		return;
	}
	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Name_new is empty @ Mesh::ChangePointSetName(...)")
		return;
	}
	auto Index_test = this->GetPointSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Name_new " << Name_new << " has been used @ Mesh::ChangePointSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->PointSetName[Index];
	m_MeshData->PointSetName[Index] = Name_new;
	auto it = m_MeshData->Map_PointSet_Name_to_Index.find(Name_old);
	m_MeshData->Map_PointSet_Name_to_Index.erase(it);
	m_MeshData->Map_PointSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangePointSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetPointSetIndex(Name_old);
	if (Index < 0 || Index >= m_MeshData->PointSet.GetLength())
	{
		MDK_Error("Name_old " << Name_old << " NOT exist @ Mesh::ChangePointSetName(...)")
		return;
	}
	this->ChangePointSetName(Index, Name_new);
}
...
void Mesh<ScalarType>::SetPointSet(const String& Name, DenseVector<int_max> PointIndexList)
{
	...
	if (Index < 0 || Index >= m_MeshData->PointSet.GetLength())
	{
		MDK_Error("Name " << Name << " NOT exist @ Mesh::SetPointSet(...)")
		return;
	}
...
void Mesh<ScalarType>::SetPointSet(int_max Index, DenseVector<int_max> PointIndexList)
{
	if (Index < 0 || Index >= m_MeshData->PointSet.GetLength())
	{
		MDK_Error("Index " << Index << " NOT exist @ Mesh::SetPointSet(...)")
		return;
	}
...
DenseVector<int_max> Mesh<ScalarType>::GetPointSet(int_max Index) const
{
	...
	if (this->Check_If_DataStructure_is_Clean() == true)
	{
		return m_MeshData->PointSet[Index];
	}
	DenseVector<int_max> PointSet;
	for (int_max k = 0; k < m_MeshData->PointSet[Index].GetLength(); ++k)
	{
		auto PointIndex = m_MeshData->PointSet[Index][k];
		if (this->IsValidPointIndex(PointIndex) == true)
		{
			PointSet.Append(PointIndex);
		}
	}
	return PointSet;
}
...
DenseVector<int_max> Mesh<ScalarType>::GetPointSet(const String& Name) const
{
	...
	if (Index < 0 || Index >= m_MeshData->PointSet.GetLength())
	{
		MDK_Error("Name " << Name << " NOT exist @ Mesh::GetPointSet(...)")
		DenseVector<int_max> EmptySet;
		return EmptySet;
	}
...
template<typename ScalarType>
void Mesh<ScalarType>::ClearPointSet(int_max Index)
{
	m_MeshData->PointSet[Index].Clear()
}

template<typename ScalarType>
void Mesh<ScalarType>::ClearPointSet(const String& Name)
{
	auto Index = this->GetPointSetIndex(Name);
	m_MeshData->PointSet[Index].Clear()
}
...
int_max Mesh<ScalarType>::AddEdgeSet(const String& Name, DenseVector<int_max> EdgeIndexList)
{
	...
	{
		m_MeshData->EdgeSet.Append(std::move(EdgeIndexList));
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangeEdgeSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->EdgeSet.GetLength())
	{
		MDK_Error("EdgeSet " << Index << " NOT exist @ Mesh::ChangeEdgeSetName(...)")
		return;
	}
	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Input Name is empty @ Mesh::ChangeEdgeSetName(...)")
		return;
	}
	auto Index_test = this->GetEdgeSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Can not change Name @ Mesh::ChangeEdgeSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->EdgeSetName[Index];
	m_MeshData->EdgeSetName[Index] = Name_new;
	auto it = m_MeshData->Map_EdgeSet_Name_to_Index.find(Name_old);
	m_MeshData->Map_EdgeSet_Name_to_Index.erase(it);
	m_MeshData->Map_EdgeSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangeEdgeSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetEdgeSetIndex(Name_old);
	this->ChangeEdgeSetName(Index, Name_new);
}
...
DenseVector<int_max> Mesh<ScalarType>::GetEdgeSet(int_max Index) const
{
	...
	DenseVector<int_max> EdgeSet;
	for (int_max k = 0; k < m_MeshData->EdgeSet[Index].GetLength(); ++k)
	{
		auto EdgeIndex = m_MeshData->EdgeSet[Index][k];
		if (this->IsValidEdgeIndex(EdgeIndex) == true)
		{
			EdgeSet.Append(EdgeIndex);
		}
	}
	return EdgeSet;
}
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangeFaceSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->FaceSet.GetLength())
	{
		MDK_Error("FaceSet " << Index << " NOT exist @ Mesh::ChangeFaceSetName(...)")
		return;
	}

	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Input Name is empty @ Mesh::ChangeFaceSetName(...)")
		return;
	}
	auto Index_test = this->GetFaceSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Can not change Name @ Mesh::ChangeFaceSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->FaceSetName[Index];
	m_MeshData->FaceSetName[Index] = Name_new;
	auto it = m_MeshData->Map_FaceSet_Name_to_Index.find(Name_old);
	m_MeshData->Map_FaceSet_Name_to_Index.erase(it);
	m_MeshData->Map_FaceSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangeFaceSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetFaceSetIndex(Name_old);
	this->ChangeFaceSetName(Index, Name_new);
}
...
DenseVector<int_max> Mesh<ScalarType>::GetFaceSet(int_max Index) const
{
	...
	DenseVector<int_max> FaceSet;
	for (int_max k = 0; k < m_MeshData->FaceSet[Index].GetLength(); ++k)
	{
		auto FaceIndex = m_MeshData->FaceSet[Index][k];
		if (this->IsValidFaceIndex(FaceIndex) == true)
		{
			FaceSet.Append(FaceIndex);
		}
	}
	return FaceSet;
}
...
template<typename ScalarType>
void Mesh<ScalarType>::ChangeCellSetName(int_max Index, const String& Name_new)
{
	if (Index < 0 || Index >= m_MeshData->CellSet.GetLength())
	{
		MDK_Error("CellSet " << Index << " NOT exist @ Mesh::ChangeCellSetName(...)")
		return;
	}
	if (Name_new.IsEmpty() == true)
	{
		MDK_Error("Input Name is empty @ Mesh::ChangeCellSetName(...)")
		return;
	}
	auto Index_test = this->GetCellSetIndex(Name_new);
	if (Index_test >= 0)
	{
		MDK_Error("Can not change Name @ Mesh::ChangeCellSetName(...)")
		return;
	}
	auto Name_old = m_MeshData->CellSetName[Index];
	m_MeshData->CellSetName[Index] = Name_new;
	auto it = m_MeshData->Map_CellSet_Name_to_Index.find(Name_old);
	m_MeshData->Map_CellSet_Name_to_Index.erase(it);
	m_MeshData->Map_CellSet_Name_to_Index[Name_new] = Index;
}


template<typename ScalarType>
void Mesh<ScalarType>::ChangeCellSetName(const String& Name_old, const String& Name_new)
{
	auto Index = this->GetCellSetIndex(Name_old);
	this->ChangeCellSetName(Index, Name_new);
}
...
DenseVector<int_max> Mesh<ScalarType>::GetCellSet(int_max Index) const
{
	...
	DenseVector<int_max> CellSet;
	for (int_max k = 0; k < m_MeshData->CellSet[Index].GetLength(); ++k)
	{
		auto CellIndex = m_MeshData->CellSet[Index][k];
		if (this->IsValidCellIndex(CellIndex) == true)
		{
			CellSet.Append(CellIndex);
		}
	}
	return CellSet;
}
...
int_max Mesh<ScalarType>::AddFaceByEdge(const DenseVector<int_max>& EdgeIndexList, int_max FaceInex_input)
{
	...
	for (int_max k = 0; k < EdgeIndexList.GetLength(); ++k)
	{
		...
		for (int_max n = k + 1; n < EdgeIndexList.GetLength(); ++n)
		{
			if (EdgeIndexList[k] == EdgeIndexList[n])
			{
				MDK_Error("EdgeIndexList is invalid : more than one copy of a point index @ Mesh::AddFaceByEdge(...)")
				return -1;
			}
		}
	}
...
{	
	...
	if (Type == MeshCellTypeEnum::Tetrahedron)
	{
		...
		{
			MDK_Error("invalid PointCount for Tetrahedron @ Mesh::AddCellByPoint(...)")
			return -1;
		}
		...
		auto FaceIndex1 = this->AddFaceByPoint({ H0, H3, H1 });
		auto FaceIndex2 = this->AddFaceByPoint({ H1, H3, H2 });
		auto FaceIndex3 = this->AddFaceByPoint({ H2, H3, H0 });
		FaceIndexList = { FaceIndex0, FaceIndex1, FaceIndex2, FaceIndex3 };		
	}
	else if (Type == MeshCellTypeEnum::Pyramid)
	{
		if (PointIndexList.GetLength() != 5)
		{
			MDK_Error("invalid PointCount for Pyramid @ Mesh::AddCellByPoint(...)")
			return -1;
		}
		auto H0 = PointIndexList[0];
		auto H1 = PointIndexList[1];
		...
		auto FaceIndex0 = this->AddFaceByPoint({ H0, H3, H2, H1 });
		auto FaceIndex1 = this->AddFaceByPoint({ H0, H1, H4 });
		auto FaceIndex2 = this->AddFaceByPoint({ H1, H2, H4 });
		auto FaceIndex3 = this->AddFaceByPoint({ H2, H3, H4 });
		auto FaceIndex4 = this->AddFaceByPoint({ H0, H4, H3 });
	...
	else if (Type == MeshCellTypeEnum::Wedge)
	{
		...
		{
			MDK_Error("invalid PointCount for Wedge @ Mesh::AddCellByPoint(...)")
			return -1;
		}
		auto H0 = PointIndexList[0];
		auto H1 = PointIndexList[1];
		...
		auto FaceIndex0 = this->AddFaceByPoint({ H0, H1, H2 });
		auto FaceIndex1 = this->AddFaceByPoint({ H3, H5, H4 });
		auto FaceIndex2 = this->AddFaceByPoint({ H0, H3, H4, H1 });
		auto FaceIndex3 = this->AddFaceByPoint({ H1, H4, H5, H2 });
		auto FaceIndex4 = this->AddFaceByPoint({ H2, H5, H3, H0 });
		FaceIndexList = { FaceIndex0, FaceIndex1, FaceIndex2, FaceIndex3, FaceIndex4 };		
	}
	else if (Type == MeshCellTypeEnum::Hexahedron)
	{
		...
		{
			MDK_Error("invalid PointCount for Hexahedron @ Mesh::AddCellByPoint(...)")
			return -1;
		}
		auto H0 = PointIndexList[0];
		auto H1 = PointIndexList[1];
		...
		auto FaceIndex1 = this->AddFaceByPoint({ H4, H7, H6, H5 });
		auto FaceIndex2 = this->AddFaceByPoint({ H0, H4, H5, H1 });
		auto FaceIndex3 = this->AddFaceByPoint({ H1, H5, H6, H2 });
		auto FaceIndex4 = this->AddFaceByPoint({ H2, H6, H7, H3 });
		auto FaceIndex5 = this->AddFaceByPoint({ H3, H7, H4, H0 });
	...
	return this->AddCellByPointAndFace(PointIndexList, FaceIndexList, CellIndex_input);	
}
...
int_max Mesh<ScalarType>::AddCellByPointAndFace(MeshCellTypeEnum Type, const DenseVector<int_max>& PointIndexList, const DenseVector<int_max>& FaceIndexList, int_max CellIndex_input)
{
	...
	for (int_max k = 0; k < PointIndexList.GetLength(); ++k)
	{
		...
		{
			if (PointIndexList[k] == PointIndexList[n])
			{
				MDK_Error("PointIndexList is invalid : more than one copy of a point index @ Mesh::AddCellByPointAndFace(...)")
				return -1;
			}
		}
	}
	...
	for (int_max k = 0; k < FaceIndexList.GetLength(); ++k)
	{
		if (this->IsValidFaceIndex(FaceIndexList[k]) == false)
		{
			MDK_Error("Invalid FaceIndexList @ Mesh::AddCellByPointAndFace(...)")
			return -1;
		}
		for (int_max n = k + 1; n < FaceIndexList.GetLength(); ++n)
		{
			if (FaceIndexList[k] == FaceIndexList[n])
			{
				MDK_Error("FaceIndexList is invalid : more than one copy of a face index @ Mesh::AddCellByPointAndFace(...)")
				return -1;
			}
		}
	}
	...
	{
		MDK_Error("CellIndex_input is out of range @ Mesh::AddCellByPointAndFace(...)")
		return -1;
	}
	...
	if (CellIndex_input < m_MeshData->CellValidityFlagList.GetLength())
	{
		...
		{
			MDK_Error("CellIndex_input is in use @ Mesh::AddCellByPointAndFace(...)")
			return -1;
		}
	}
	...
	{
		//MDK_Warning("The cell has been added already @ Mesh::AddCellByPointAndFace(...)")
		...
		{// attempt to replace old&valid cell with new cell
			MDK_Error("Cell arealdy exists at CellIndex: " << CellIndex_temp << " @ Mesh::AddCellByPointAndFace(...)")
			return -1;
		}
	}
...
Mesh<ScalarType>::GetSubMeshByFace(const DenseVector<int_max>& FaceIndexList) const
{
	...
	{		
		for (int_max Index = 0; Index < m_MeshData->PointDataSet.GetLength(); ++Index)
		{
			const auto& Name = m_MeshData->PointDataSetName[Index];
			const auto& DataSet_in = m_MeshData->PointDataSet[Index];
			DenseMatrix<ScalarType> DataSet_out;			
			DataSet_out.Resize(DataSet_in.GetRowCount(), OutputMesh.GetPointCount());
			...
			OutputMesh.AddPointDataSet(Name, DataSet_out);
		}
	}
	...
	{		
		for (int_max Index = 0; Index < m_MeshData->EdgeDataSet.GetLength(); ++Index)
		{
			const auto& Name = m_MeshData->EdgeDataSetName[Index];
			...
			OutputMesh.AddEdgeDataSet(Name, DataSet_out);
		}
	}
	...
	{		
		for (int_max Index = 0; Index < m_MeshData->FaceDataSet.GetLength(); ++Index)
		{
			const auto& Name = m_MeshData->FaceDataSetName[Index];
			...
			OutputMesh.AddFaceDataSet(Name, DataSet_out);
		}
	}
...
Mesh<ScalarType> Mesh<ScalarType>::GetSubMeshByCell(const DenseVector<int_max>& CellIndexList) const
{
	...
	if (this->GetCellDataSetCount() > 0)
	{
		for (int_max Index = 0; Index < m_MeshData->CellDataSet.GetLength(); ++Index)
		{
			const auto& Name = m_MeshData->CellDataSetName[Index];
			...
			OutputMesh.AddCellDataSet(Name, DataSet_out);
		}
	}
...
{// append a mesh, duplication of face/edge/point will not be checked
 // this is not checked: append non-triangle mesh to triangle mesh
	...
	for (int_max k = 0; k < InputMesh.m_MeshData->PointSet.GetLength(); ++k)
	{
		...
		auto PointSet = InputMesh.m_MeshData->PointSet[k];
		PointSet += MaxPointIndex_init + 1;
		this->AddPointSet(PointSetName, PointSet);
	}

	for (int_max k = 0; k < InputMesh.m_MeshData->EdgeSet.GetLength(); ++k)
	{
		auto EdgeSetName = InputMesh.m_MeshData->EdgeSetName[k];
		auto EdgeSet = InputMesh.m_MeshData->EdgeSet[k];
		EdgeSet += MaxEdgeIndex_init + 1;
		this->AddEdgeSet(EdgeSetName, EdgeSet);
	}

	for (int_max k = 0; k < InputMesh.m_MeshData->FaceSet.GetLength(); ++k)
	{
		auto FaceSetName = InputMesh.m_MeshData->FaceSetName[k];
		auto FaceSet = InputMesh.m_MeshData->FaceSet[k];
		FaceSet += MaxFaceIndex_init + 1;
		this->AddFaceSet(FaceSetName, FaceSet);
	}

	for (int_max k = 0; k < InputMesh.m_MeshData->CellSet.GetLength(); ++k)
	{
		auto CellSetName = InputMesh.m_MeshData->CellSetName[k];
		auto CellSet = InputMesh.m_MeshData->CellSet[k];
		CellSet += MaxCellIndex_init + 1;
		this->AddCellSet(CellSetName, CellSet);
	}

	//TODO: merge dataset
	//for now, just clear dataset
	//Clear PointDataSet/EgeDataSet/FaceDataSet/CellDataSet
	if (m_MeshData->PointDataSet.IsEmpty() == false)
	{
		MDK_Warning("PointDataSet is cleared @ Mesh::Append(...)")
		m_MeshData->PointDataSet.Clear();
		m_MeshData->PointDataSetName.Clear();
		m_MeshData->Map_PointDataSet_Name_to_Index.clear();
	}
	if (m_MeshData->EdgeDataSet.IsEmpty() == false)
	{
		MDK_Warning("EdgeDataSet is cleared @ Mesh::Append(...)")
		m_MeshData->EdgeDataSet.Clear();
		m_MeshData->EdgeDataSetName.Clear();
		m_MeshData->Map_EdgeDataSet_Name_to_Index.clear();
	}
	if (m_MeshData->FaceDataSet.IsEmpty() == false)
	{
		MDK_Warning("FaceDataSet is cleared @ Mesh::Append(...)")
		m_MeshData->FaceDataSet.Clear();
		m_MeshData->FaceDataSetName.Clear();
		m_MeshData->Map_FaceDataSet_Name_to_Index.clear();
	}
	if (m_MeshData->CellDataSet.IsEmpty() == false)
	{
		MDK_Warning("CellDataSet is cleared @ Mesh::Append(...)")
		m_MeshData->CellDataSet.Clear();
		m_MeshData->CellDataSetName.Clear();
		m_MeshData->Map_CellDataSet_Name_to_Index.clear();
	}
