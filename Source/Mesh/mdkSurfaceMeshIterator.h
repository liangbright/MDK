...
//================================================= Iterator_Of_Vertex_Of_SurfaceMesh ==================================//

template<typename MeshType>
struct Iterator_Of_Vertex_Of_SurfaceMesh
{
private:
    SurfaceMesh<MeshType> m_Mesh;
    Handle_Of_Vertex_Of_SurfaceMesh m_VertexHandle;

    //----------------------------------
public:
    Iterator_Of_Vertex_Of_SurfaceMesh(SurfaceMesh<MeshType>& ParentMesh)
    {
        m_Mesh.Share(ParentMesh);
        this->SetToBegin();
    }
    ...
    Handle_Of_Vertex_Of_SurfaceMesh GetVertexHandle() const
    {
    ...
    void operator+=(int_max Offset)
    {
        ...
        {
            auto VertexIndex = m_VertexHandle.GetIndex();
            if (VertexIndex + Offset < m_Mesh.m_MeshData->VertexList.GetLength())
            {
                int_max Counter = 0;
                int_max VertexIndex_next = -1;
                for (int_max k = VertexIndex + 1; k < m_Mesh.m_MeshData->VertexList.GetLength(); ++k)
                {
                    if (m_Mesh.m_MeshData->VertexList[k].IsValid() == true)
                    {
                        Counter += 1;
                    }
                ...
                m_VertexHandle.SetIndex(VertexIndex_next);
            }
            else
            {
                m_VertexHandle.SetIndex(-1);
            }
        }
        else
        {
            ...
            if (VertexIndex + Offset >= 0)
            {
                int_max Counter = 0;
                int_max VertexIndex_next = -1;
                for (int_max k = VertexIndex - 1; k >= 0; --k)
                {
                    if (m_Mesh.m_MeshData->VertexList[k].IsValid() == true)
                    {
                        Counter += 1;
    ...
    {
        auto VertexIndex = m_VertexHandle.GetIndex();
        return (VertexIndex >= 0 && VertexIndex < m_Mesh.m_MeshData->VertexList.GetLength());
    }

    void SetToBegin()
    {
        int_max VertexIndex_begin = -1;
        for (int_max k = 0; k < m_Mesh.m_MeshData->VertexList.GetLength(); ++k)
        {
            if (m_Mesh.m_MeshData->VertexList[k].IsValid() == true)
            {
                VertexIndex_begin = k;
        ...
        m_VertexHandle.SetIndex(VertexIndex_begin);
    }
};
